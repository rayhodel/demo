...
1. End-to-End Functional Testing
   1.1. Start application with `dotnet run`
   1.2. Open browser to application URL
   1.3. Verify home page loads successfully
   1.4. Verify settings button is visible
   1.5. Click settings button, verify modal opens
   1.6. Enter test MCommunity API credentials
   1.7. Click Save, verify modal closes
   1.8. Refresh page, verify credentials persist

2. Test Person Lookup Scenarios
   2.1. Enter valid uniqname (3-8 alphanumeric chars)
   2.2. Click Search button, verify loading indicator appears
   2.3. Verify person results display with all fields populated
   2.4. Test with uniqname that has multiple emails/phones/titles
   2.5. Verify arrays are displayed correctly
...
3. Test Group Lookup Scenarios
   3.1. Enter valid group name (9-62 chars, alphanumeric with dash/underscore)
   3.2. Click Search button, verify loading indicator appears
   3.3. Verify group results display with all fields populated
...
4. Test Auto-Detection Logic
   4.1. Enter 5-character alphanumeric string, verify detected as person
   4.2. Enter 15-character alphanumeric string, verify detected as group
...
12. Create README for Project Root
